...
def check_R1(campaign: dict, cwd: Path) -> CheckResult:
    ...
    if path.is_dir():
        return CheckResult("R1", True, message=f"{_ML_METAOPT_DIR}/ directory exists")
    return CheckResult(
        "R1",
        False,
        message=f"{_ML_METAOPT_DIR}/ directory is missing",
        remediation=f"Create the directory: mkdir -p {_ML_METAOPT_DIR}",
    )


def check_R2(campaign: dict, cwd: Path) -> CheckResult:
    """Root .gitignore contains '.ml-metaopt/' entry."""
    ...
    if not gitignore.is_file():
        return CheckResult(
            ...
            remediation=f"Create .gitignore containing '{_ML_METAOPT_DIR}/'",
        )
    ...
    return CheckResult(
        "R2",
        False,
        message=f".gitignore exists but does not contain '{_ML_METAOPT_DIR}/'",
        remediation=f"Add '{_ML_METAOPT_DIR}/' to .gitignore",
    )
...
def check_R3(campaign: dict, cwd: Path) -> CheckResult:
    ...
    if path.is_dir():
        return CheckResult("R3", True, message=f"{_ML_METAOPT_DIR}/handoffs/ exists")
    return CheckResult(
        "R3",
        False,
        message=f"{_ML_METAOPT_DIR}/handoffs/ is missing",
        remediation=f"Create the directory: mkdir -p {_ML_METAOPT_DIR}/handoffs",
    )
...
def check_R4(campaign: dict, cwd: Path) -> CheckResult:
    ...
    if path.is_dir():
        return CheckResult("R4", True, message=f"{_ML_METAOPT_DIR}/worker-results/ exists")
    return CheckResult(
        "R4",
        False,
        message=f"{_ML_METAOPT_DIR}/worker-results/ is missing",
        remediation=f"Create the directory: mkdir -p {_ML_METAOPT_DIR}/worker-results",
    )
...
def check_R5(campaign: dict, cwd: Path) -> CheckResult:
    ...
    if not missing:
        return CheckResult("R5", True, message=f"{_ML_METAOPT_DIR}/tasks/ and executor-events/ exist")
    return CheckResult(
        "R5",
        False,
        message=f"Missing subdirectories in {_ML_METAOPT_DIR}/: {', '.join(missing)}",
...
def check_R6(campaign: dict, cwd: Path) -> CheckResult:
    ...
    for subdir in subdirs:
        if not (cwd / _ML_METAOPT_DIR / subdir).is_dir():
            missing.append(subdir)
    if not missing:
        return CheckResult("R6", True, message=f"All {_ML_METAOPT_DIR}/artifacts/ subdirectories exist")
    return CheckResult(
        "R6",
        False,
        message=f"Missing artifact subdirectories in {_ML_METAOPT_DIR}/: {', '.join(missing)}",
        remediation="Create missing directories: "
        + " ".join(f"mkdir -p {_ML_METAOPT_DIR}/{d}" for d in missing),
    )
...
def check_R7(campaign: dict, cwd: Path) -> CheckResult:
    ...
    if not isinstance(project, dict):
        return CheckResult(
            "R7",
            False,
            message="Campaign YAML missing 'project' section (needed for smoke_test_command)",
            remediation="Add a 'project' section with a 'smoke_test_command' string to the campaign YAML",
        )
    cmd = project.get("smoke_test_command")
    if isinstance(cmd, str) and cmd.strip():
        return CheckResult("R7", True, message="smoke_test_command is present and non-empty")
    return CheckResult(
        "R7",
        False,
        message="smoke_test_command is missing or empty",
        remediation="Set project.smoke_test_command to a valid shell command string",
    )
...
def check_R8(campaign: dict, cwd: Path) -> CheckResult:
    ...
    if not missing:
        return CheckResult("R8", True, message="All required top-level campaign keys are present")
    sorted_missing = sorted(missing)
    return CheckResult(
        "R8",
        False,
        message=f"Missing required top-level keys: {', '.join(sorted_missing)}",
        remediation=f"Add the following keys to the campaign YAML: {', '.join(sorted_missing)}",
    )
...
def check_R9(campaign: dict, cwd: Path) -> CheckResult:
    ...
    if not isinstance(project, dict):
        return CheckResult(
            "R9",
            False,
            message="Campaign YAML missing 'project' section (needed for project.repo)",
            remediation="Add a 'project' section with a 'repo' string to the campaign YAML",
        )
    repo = project.get("repo")
    if isinstance(repo, str) and repo.strip():
        return CheckResult("R9", True, message="project.repo is present and non-empty")
    return CheckResult(
        "R9",
        False,
        message="project.repo is missing or empty",
        remediation="Set project.repo to the repository URL or path",
    )
